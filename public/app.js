...
function renderQ(target){
  ...
  box.innerHTML=current.map((q,i)=>`<article class="question-card" data-id="${esc(q.id)}"><div class="question-meta">${i+1}. ${esc(q.subject)} · ${safeArr(q.tags).map(esc).join(' · ')}</div><h4>${esc(cleanText(q.question))}</h4>${Array.isArray(q.statements)&&q.statements.length?`<div class="question-statements"><strong>보기</strong><ol class="statement-list">${q.statements.map((s,n)=>`<li><span class="statement-label">${['ㄱ','ㄴ','ㄷ','ㄹ','ㅁ','ㅂ','ㅅ','ㅇ'][n]||`${n+1}`}</span>${esc(cleanText(s))}</li>`).join('')}</ol></div>`:''}<div>${q.choices.map((c,n)=>`<label class="choice"><input type="radio" name="${esc(q.id)}" value="${n}"><span>${n+1}. ${esc(cleanText(c))}</span></label>`).join('')}</div><div class="explain"><strong>정답 ${answerLabel(q.answer)}</strong><br>${esc(cleanText(q.explain))}</div></article>`).join('');
