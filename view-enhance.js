(function enhanceViews(){
  ...
  const esc=(value)=>extraCleanText2(value).replace(/발달와/g,'발달과').replace(/지역복지(['’])을/g,'지역복지$1를').replace(/참며자/g,'참여자').replace(/비교\s+한다/g,'비교한다').replace(/정도를평가/g,'정도를 평가').replace(/조사시/g,'조사 시').replace(/표집를선정/g,'표집단위 선정').replace(/[&<>\"]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[m]));
  ...
  function renderPastDetails(){
    ...
    const rows=filtered.slice(0,60).map((q,idx)=>`<article class="question-card always-open"><div class="question-meta">${idx+1}. ${esc(yearOf(q)||'연도 미상')}년 · ${esc(q.period||'교시 미상')} · ${esc(q.subject)} ${q.number?`· ${esc(q.number)}번`:''}</div><h4>${esc(q.question)}</h4>${statementMarkup(q)}<div class="choices-view">${q.choices.map((c,n)=>`<div class="choice ${Number(q.answer)===n?'correct-choice':''}"><span>${answerLabel(n)}</span><span>${esc(c)}</span></div>`).join('')}</div><div class="explain" style="display:block"><strong>정답 ${answerLabel(q.answer)}</strong><br>${esc(explainOf(q))}</div></article>`).join('');
  ...
  function renderAnswerDetails(){
    ...
    const rows=filtered.slice(0,80).map(q=>`<article class="answer-detail-card"><p class="question-meta">${esc(yearOf(q)||'연도 미상')}년 · ${esc(q.period||'교시 미상')} · ${esc(q.subject)} ${q.number?`· ${esc(q.number)}번`:''}</p><h4>${q.number?`${esc(q.number)}번. `:''}${esc(q.question)}</h4>${statementMarkup(q)}<div class="choices-view">${q.choices.map((c,n)=>`<div class="answer-choice ${Number(q.answer)===n?'correct-choice':''}"><strong>${answerLabel(n)}</strong><span>${esc(c)}</span></div>`).join('')}</div><div class="answer-note"><strong>정답 ${answerLabel(q.answer)}</strong><br>${esc(explainOf(q))}</div></article>`).join('');
