(function boostQuestionsFromSummaries(){
  ...
  guides.forEach((guide,index)=>{
    ...
    generated.push({
      id:idBase+'-def', subject, tags:tagOf(guide,'정의'),
      question:`${title}에 대한 설명으로 가장 적절한 것은?`,
      choices:[clean(guide.oneLine||core[0]||memory), wrong[0], wrong[1], wrong[2], wrong[3]],
      answer:0,
      explain:`${title}: ${memory}`
    });
    generated.push({
      id:idBase+'-exam', subject, tags:tagOf(guide,'빈출'),
      question:`${subject} 영역에서 '${title}'을 판단할 때 가장 중요한 기출 포인트는?`,
      choices:[exam, `${title}은 항상 법률상 급여액 계산 문제로만 출제된다.`, `${title}은 지역사회복지론에서만 다루는 개념이다.`, '개념 이름보다 암기 순서를 무조건 우선한다.', '모든 보기에서 개인 책임만 확인하면 된다.'],
      answer:0,
      explain:`기출 포인트: ${exam}`
    });
    generated.push({
      id:idBase+'-trap', subject, tags:tagOf(guide,'오답함정'),
      question:`다음 중 '${title}'과 관련해 오답으로 의심해야 할 설명은?`,
      choices:[trap, core[0]||memory, core[1]||exam, memory, `${unit} 단원에서 함께 비교할 수 있다.`],
      answer:0,
      explain:`오답 함정: ${trap}`
    });
    generated.push({
      id:idBase+'-memory', subject, tags:tagOf(guide,'암기'),
      question:`'${title}'을 빠르게 암기하기 위한 문장으로 가장 적절한 것은?`,
      choices:[memory, wrong[1], wrong[2], '정답 선택지는 항상 가장 긴 문장이다.', '법제 과목의 세부 시행일만 외우면 충분하다.'],
      answer:0,
      explain:`암기 문장: ${memory}`
    });
  });
